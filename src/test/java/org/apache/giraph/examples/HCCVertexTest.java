...
public class HCCVertexTest extends TestCase {

    public void testCombiner() throws Exception {

        VertexCombiner<IntWritable,IntWritable> combiner =
                new HCCVertex.HCCVertexCombiner();
    ...
    public void testToyData() throws Exception {
        ...
        Iterable<String> results = InternalVertexRunner.run(
                HCCVertex.class,
                HCCVertex.HCCVertexCombiner.class,
                SymmetricGraphTextInputFormat.class,
                HCCVertexOutputFormat.class,
        ...
        assertEquals(7, components.get(1).size());
        assertEquals(5, components.get(6).size());
        assertEquals(1, components.get(9).size());
    }
    ...
    private SetMultimap<Integer,Integer> parseResults(
        ...
        for (String result : results) {
            ...
            int vertexID = Integer.parseInt(Iterables.get(parts, 0));
            int componentID = Integer.parseInt(Iterables.get(parts, 1));
            components.put(componentID, vertexID);
        }
        return components;
    }
}
